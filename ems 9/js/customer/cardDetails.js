...
if (session) {
  ...
  /* Nothing to pay (the page was opened directly, or the bill has since
     been settled) → back to the bill list. Setting location does not
     stop this script, so the rest of the page is wired up in the else
     branch, where `bill` is known to exist.                          */
  if (!bill || bill.status === "Paid") {
    ...
    document.getElementById("payBtn").addEventListener("click", () => {
      ...
      if (!payment) {                       // already paid in another tab
