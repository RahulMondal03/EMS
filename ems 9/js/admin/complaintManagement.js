    ...
    rows.innerHTML = complaints.map(k => {
      ...
      if (k.status === "Open" && canAssign && team.length) {
        const options = team.map(e =>
          `<option value="${e.id}">${EMS.esc(e.name)} · ${EMS.esc(EMS.positionLabel(e))}</option>`).join("");
        assignCell = `
        ...
        assignCell = assignee ? `${EMS.esc(assignee.name)} · ${EMS.esc(EMS.positionLabel(assignee))}` : "—";
      }
      ...
      return `
        <tr>
          ...
          <td style="max-width:280px;">${EMS.esc(k.description)}</td>
